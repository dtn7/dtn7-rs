...
$DIR/../target/$TARGET/dtnd -d -j5s -i0 -w $PORT_NODE1 -C http -e incoming -r epidemic -n node1 -s http://127.0.0.1:$PORT_NODE2/node2 $DB1 $STATUS_REPORTS 2>&1 &> $OUT_NODE1 &
PID_NODE1=$!
...
$DIR/../target/$TARGET/dtnd -d -j5s -i0 -w $PORT_NODE2 -C http -e incoming -r epidemic \
    -n node2 \
